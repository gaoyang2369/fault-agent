...
- 数据库/表：`dcma.real_data`
- 数据类型：模拟历史运行数据
- 名义采样周期：约 3 秒
- 2026-07-16 摸底快照：566 条，`timestamp` 566/566 可按 13 位 Unix 毫秒时间戳解析
- 实测采样间隔：P50 3.196 秒、P95 6.3936 秒，存在一处 13005.709 秒的大间隔
- 当前没有实时采集链路
- M0/M1 对该表只读访问
...
| `12_data_profile_findings.md` | 2026-07-16 只读数据摸底事实与决策影响 |
...
- `fault_code` / `alarm_code` 无事件值的领域确认和产品语义（样本仅观测到候选编码）；
- 厂家手册及其适用版本；
- 一段经确认的健康基线数据。
...
2026-07-16 摸底已观测到 `fault_code` 的 `0` / `F1030-0/0/0` 和 `alarm_code` 的全量 `0`，但这只解决了“样本中出现哪些原始值”，没有解决无事件值的领域确认、G120 精确变体、固件和手册适用性。因此设备事件规则继续保持 `enabled: false`，不得根据本次摸底自动启用或解释故障码。
...
<!-- Source: 12_data_profile_findings.md -->
...
- `device_name` 和 `inverter_name` 的唯一值均为 `G120电机1`；
- 两者与领域资产 `G120-1` 的正式映射仍为 `TODO-DOMAIN`；
- 名称相同不能证明电机与变频器是同一部件，仍须分离 `Motor` 与 `Inverter`。

## 3. 时间编码与采样

- `timestamp` 为 13 位 Unix 毫秒时间戳，566/566 可解析为带时区的 UTC 时间点；
- `date` 格式为 `YYYY/MM/DD`，`time` 格式为 `HH:mm:ss SSSms`；
- `date + time` 和无时区 `create_time` 的源时区仍未确认；
- 相邻间隔最小值 3.017 秒，P50 3.196 秒，P95 6.3936 秒；
- 最大间隔 13005.709 秒，持续时间分析不得跨越该大间隔。
...
`0` 只是候选无事件值；`F1030-0/0/0` 只作为原始上报码保留；状态字和控制字未确认编码与位定义前禁止解释。本次摸底不启用设备事件规则。

## 5. 数值字段质量与冻结候选

18 个数值信号均有 566 个有限数值，缺失/非法率为 0，但单位和缩放继续保持未知。不得依据样本范围创建工业阈值。

`alarm_code`、`torque_actual`、`air_intake_temp`、`feedback_power` 在本快照中全量固定。其中 `air_intake_temp = -200` 在单位、缩放和占位约定确认前，不能解释为实际温度或传感器故障。

## 6. 对 M1 的影响

- 原生支持 13 位 Unix 毫秒 `timestamp` 并生成 UTC timezone-aware `observed_at`；
- `date + time` 仅作为回退，且依赖显式 `SOURCE_TIMEZONE`；
- 持续时间分析按数据大间隔切分；
- 资产映射、事件编码、单位、缩放、手册版本和健康基线仍是开放项；
- 依赖单位或产品语义的规则继续保持 `enabled: false`。
